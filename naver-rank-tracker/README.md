...
| 파일 | 용도 |
|---|---|
| `실행.bat` | 자동 설치 후 서버 시작 — 콘솔에 접속 주소를 안내하고 브라우저를 자동으로 엽니다 |
| `진단.bat` | 실환경 자가진단 — 뭐가 되고 안 되는지 항목별 ✅/❌ 표시 |
| `선택기능설치.bat` | 실측 검증·쿠팡 차단 우회용 실브라우저 설치 |
| `D드라이브로이사.bat` | 폴더를 `D:\자동화프로그램\순위추적기` 로 옮김 (순위 데이터도 함께) |

> 배치 파일 안내문은 영문입니다 — 윈도우 cmd가 배치 파일의 한글을 깨뜨리는 문제를
> 피하기 위해 한글 안내는 모두 파이썬 쪽에서 출력합니다.
...
tests/run_all.py   # 검증 스크립트 일괄 실행 (10개 스위트)
```
...
- 상단 상태바: 마지막 조회 시각·성공/실패 건수·다음 자동 조회 시각
- 키워드 행: 순위와 함께 **언제 측정된 것인지**(오늘 / N일 전) 표시.
  조회에 실패한 날은 '조회 실패'로 남아 옛 순위가 최신처럼 보이지 않는다
- 키워드는 상품별로 **추가·삭제 가능** (기존 순위 이력은 그대로 유지)
