...
`tests/` 폴더에 검증 스크립트 8종 (총 82건, 모두 통과 상태로 인계):
...
- `test_stage4.py` — 배포 준비 (WEB_PASSWORD 인증·DB 경로 오버라이드)
- `test_stage5.py` — 파서 내구성 (클래스 전면 변경·키 이름 변경·JSON 대체 경로)

## 8. ⚠️ 알려진 한계 / 실환경 미검증 항목 (가장 중요)

1. **쿠팡·네이버 실측의 실제 페이지 파싱은 미검증** — 개발 환경이 외부망 차단이라 픽스처로만 검증함. **PC 첫 실행 때 `진단.bat`(= `python doctor.py`)으로 실환경 확인 필수.**
   파서는 구조 변경에 견디도록 다중 전략으로 작성됨(쿠팡: `data-product-id` 앵커 → 내장 JSON / 네이버: `__NEXT_DATA__` → 다른 script JSON, ID·제목 키 이름 후보 다수). 그래도 실패하면 `coupang_debug.html`/`naver_debug.html` 자동 저장 + 페이지 성격 진단 문구(차단/캡차/로그인/결과없음/구조변경)를 함께 출력하므로 그 기준으로 조정.
...
1. **PC에서 실환경 스모크 테스트** ← 유일하게 남은 필수 항목.
   지원 도구 준비됨: `진단.bat`(= `python doctor.py`)이 파이썬·패키지·네이버 키 유효성(실호출)·
   쿠팡 접속/파싱·선택기능·텔레그램을 항목별 ✅/❌로 점검하고 해결법을 출력.
   실패 시 그 출력(+자동 생성되는 `*_debug.html`)만 공유하면 원격 진단 가능
2. ~~서버 배포 준비~~ **완료** — Dockerfile + `WEB_PASSWORD` Basic 인증 + `/data` 볼륨. 절차는 `DEPLOY.md`
3. ~~순위 추이 그래프~~ **완료** — 키워드 클릭 시 SVG 라인 차트 (순위축 반전, 미발견 선 끊김, API/실측 2계열, 호버 툴팁, 라이트·다크 색상 접근성 검증됨)
